...
def image_game_field(visual_field, user, current_score, max_score, game_over):
    ...
    for i in range(4):
        for j in range(4):
            ...
            text = 'WASD for control'
            cv2.putText(frame, text, (15, 700), font, 1, (0, 0, 0), 1, cv2.LINE_AA)

            text = 'Press "Z" for undo step'
            cv2.putText(frame, text, (15, 735), font, 1, (0, 0, 0), 1, cv2.LINE_AA)

            text = 'Press "5" for new game'
            cv2.putText(frame, text, (15, 770), font, 1, (0, 0, 0), 1, cv2.LINE_AA)

            text = 'Player: ' + str(user)
            cv2.putText(frame, text, (30, 825), font, 1, (0, 0, 0), 1, cv2.LINE_AA)

            text = 'Score: ' + str(current_score) + '   Best score: ' + str(max_score)
            cv2.putText(frame, text, (30, 865), font, 1, (0, 0, 0), 1, cv2.LINE_AA)
    ...
    if game_over:
        ...
        text = 'Press "5" for new game'
        cv2.putText(frame, text, (120, 400), font, 1, (255, 255, 255), 2, cv2.LINE_AA)

        text = 'Press "Esc" to exit'
        cv2.putText(frame, text, (165, 440), font, 1, (255, 255, 255), 2, cv2.LINE_AA)
